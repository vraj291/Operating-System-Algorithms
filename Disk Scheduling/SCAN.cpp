...
class SCAN{
    ...
    SCAN(int arr[],int len,int pos):start(pos),requestCount(len){
    ...
    void schedule(){
        ...
        outputSequence.push_back(aux[currIndex]);
        cylindersTraversed += abs(aux[currIndex]-aux[startIndex+1]);
        currIndex = startIndex+1;

        for(i=currIndex;i<aux.size()-2;i++){
            outputSequence.push_back(aux[currIndex]);
            cylindersTraversed += abs(aux[currIndex]-aux[currIndex+1]);
            currIndex++;
        }
